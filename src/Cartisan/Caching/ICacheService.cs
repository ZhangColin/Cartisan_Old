...
namespace Cartisan.Caching {
    ...
    public interface ICacheService {
        ...
        /// <param name="cacheKey">缓存项标识</param>
        /// <returns>返回cacheKey对应的缓存项，如果不存在则返回 T 的默认值</returns>
        T Get<T>(string cacheKey);

        /// <summary>
        /// 从缓存获取，如果缓存不存在，则使用数据获取方法获取数据并缓存
        /// </summary>
        /// <typeparam name="T">缓存项类型</typeparam>
        /// <param name="cacheKey">缓存项标识</param>
        /// <param name="acquire">数据获取方法</param>
        /// <returns>返回cacheKey对应的缓存项，如果不存在则返回 T 的默认值</returns>
        ...
        /// <param name="cachingExpirationType">缓存期限类型</param>
        /// <returns>返回cacheKey对应的缓存项，如果不存在则返回 T 的默认值</returns>
        T Get<T>(string cacheKey, Func<T> acquire, CachingExpirationType cachingExpirationType);
        ...
        /// <param name="timeSpan">缓存失效时间</param>
        /// <returns>返回cacheKey对应的缓存项，如果不存在则返回 T 的默认值</returns>
        T Get<T>(string cacheKey, Func<T> acquire, TimeSpan timeSpan);

...
namespace Cartisan.Authorization {
    ...
    public class CartisanUserManager: UserManager<CartisanUser, long> {
        ...
        public static CartisanUserManager Create(IdentityFactoryOptions<CartisanUserManager> options, IOwinContext context) {
            ...
            // 配置用户名的验证逻辑
            manager.UserValidator = new UserValidator<CartisanUser, long>(manager) {
            ...
            manager.RegisterTwoFactorProvider("电话代码", new PhoneNumberTokenProvider<CartisanUser, long> {
                MessageFormat = "你的安全代码是 {0}"
            });
            manager.RegisterTwoFactorProvider("电子邮件代码", new EmailTokenProvider<CartisanUser, long> {
                Subject = "安全代码",
